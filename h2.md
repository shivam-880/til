...
2. Create TCP and Web servers
    ...
    **Notes:** 
    ...
    - In **embedded mode**, it is not required to create a TCP server. However, not starting TCP server would require the specification of absolute path of the database in the jdbc url in console, which could be a bit inconvenient. Alternatively, we could start TCP server alonside Web server while accessing H2 in embedded mode from the app and in TCP from the console.
